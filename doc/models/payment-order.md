...
| `ultimateOriginatingPartyName` | `string \| null` | Required | Name of the ultimate originator of the payment order. |
| `ultimateOriginatingPartyIdentifier` | `string \| null` | Required | Identifier of the ultimate originator of the payment order. |
| `ultimateReceivingPartyName` | `string \| null` | Required | - |
...
| `transactionMonitoringEnabled` | `boolean` | Required | A flag that determines whether a payment order should go through transaction monitoring. |
| `complianceRuleMetadata` | `unknown \| null` | Required | Custom key-value pair for usage in compliance rules. Please contact support before making changes to this field. |
| `referenceNumbers` | [`PaymentReference[]`](../../doc/models/payment-reference.md) | Required | - |
| `vendorFailureReason` | `string \| null` | Required | This field will be populated if a vendor (e.g. Currencycloud) failure occurs. Logic shouldn't be built on its value as it is free-form. |
| `decisionId` | `string \| null` | Required | The ID of the compliance decision for the payment order, if transaction monitoring is enabled. |
